...
public partial class cadastropj : System.Web.UI.Page
{
    ...
    protected void btnEnviarCadastro_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            "ELSE IF EXISTS (SELECT CPF_CNPJ FROM tblDoador WHERE CPF_CNPJ LIKE @cpfcnpj) BEGIN SELECT 'CNPJ já cadastrado' END " +
            "ELSE BEGIN INSERT INTO tblDoador(Email,Senha,Pergunta,Resposta,CPF_CNPJ,RazaoSocial,TelefoneA,TelefoneB,Logradouro,Numero,Complemento,CEP,Ativo,Uf,Cidade,Bairro) " +
            "VALUES(@email,@senha,@pergunta,@resposta,@cpfcnpj,@razao,@tel1,@tel2,@logra,@numero,@comple,@cep,@ativo,@uf,@cidade,@bairro) END SELECT 'Você é um doador agora :)'";
            ...
            conexao.command.Parameters.Add("@cpfcnpj", SqlDbType.VarChar).Value = cnpj;
